...
// outlive the argv.
uint64_t extractCoreMask(const std::string &raw,
...
void mergeInto(ProcessingContext &dst, const ProcessingContext &src) {
  for (const auto &kv : src.protocolData) {
    ...
    out.connections.insert(kv.second.connections.begin(),
                           kv.second.connections.end());
...
int run_dpdk_capture(ProcessingContext &ctx) {
  ...
  pcpp::CoreMask workerMask =
      coreMask & ~static_cast<pcpp::CoreMask>(1ULL << 0);
  ...
  if (numWorkers <= 1) {
    ...
    if (!dev->openMultiQueues(static_cast<uint16_t>(numWorkers), 1)) {
      std::cerr << "Cannot open " << numWorkers << " RX queues on port "
                << FLAGS_dpdk_port
                << " (NIC may not support that many / RSS)" << std::endl;
      return 1;
    }
    if (!dev->startCaptureMultiThreads(onDpdkPacketsArrive, &shared,
                                       workerMask)) {
  ...
  std::cout << "Capturing from DPDK port " << FLAGS_dpdk_port << " on "
            << (numWorkers <= 1 ? 1 : numWorkers)
            << " worker(s) (Ctrl-C to stop)" << std::endl;
